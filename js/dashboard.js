(function () {
  ...
  let shareViewData = null;
  let calcProof = null;
  let oauthStatusMap = {};
  const SHARE_PLATFORMS = new Set(['instagram', 'tiktok']);
  ...
  function needsOAuthBar() {
    return [...selectedPlatforms].some((p) => SHARE_PLATFORMS.has(p));
  }

  async function loadOAuthStatus() {
    if (!serverOk) return;
    try {
      const data = await apiFetch(`/api/oauth/status?sessionId=${encodeURIComponent(sessionId())}`, {
        headers: { 'X-Session-Id': sessionId() },
      });
      if (data.ok) oauthStatusMap = data.platforms || {};
    } catch {
      oauthStatusMap = {};
    }
    renderOAuthBar();
  ...
  function renderOAuthBar() {
    const bar = document.getElementById('oauth-bar');
    if (!bar) return;
    if (!needsOAuthBar()) {
    ...
    ['instagram', 'tiktok'].forEach((pid) => {
      const btn = document.getElementById(`btn-oauth-${pid === 'instagram' ? 'instagram' : 'tiktok'}`);
      if (!btn) return;
      ...
      btn.hidden = false;
      const st = oauthStatusMap[pid] || {};
      if (st.connected) {
        btn.textContent = `${pid === 'instagram' ? 'Instagram' : 'TikTok'} ✓ ${st.username || ''}`.trim();
        btn.classList.add('connected');
      } else {
        btn.textContent = pid === 'instagram' ? 'Instagram 연결' : 'TikTok 연결';
  ...
  function connectOAuth(platform) {
    const sid = sessionId();
    location.href = `/oauth/${platform}/start?sessionId=${encodeURIComponent(sid)}`;
  }

  function handleOAuthReturn() {
    const qs = new URLSearchParams(location.search);
    if (qs.get('oauth_ok') === '1') {
      history.replaceState({}, '', location.pathname);
      loadOAuthStatus();
    }
    if (qs.get('oauth_error')) {
      history.replaceState({}, '', location.pathname);
      showConnError('플랫폼 연결에 실패했습니다. 다시 시도하세요.');
    }
  }

  function oauthReadyForShare(items) {
    return items.every((pid) => {
      if (!SHARE_PLATFORMS.has(pid)) return true;
      return oauthStatusMap[pid]?.connected;
    });
  }

  async function checkConnection() {
    if (location.protocol === 'file:') {
      showConnError('크라잇에이터시작.bat 실행 후 http://localhost:8100 으로 접속하세요.');
      return false;
    }
    try {
  ...
  function renderSharePanel(entries, creatorName) {
    const bar = document.getElementById('share-bar');
    const openLink = document.getElementById('share-open-link');
    const msg = document.getElementById('share-msg');
    const socialSlot = document.getElementById('share-social-slot');
    const shareItems = (entries || []).filter((e) => SHARE_PLATFORMS.has(e.platformId) && e.gross > 0);
    if (!bar) return;
    openLink?.classList.add('hidden');
    msg?.classList.add('hidden');
    socialSlot?.classList.add('hidden');
    if (socialSlot) socialSlot.innerHTML = '';
    ...
    sharePayload = {
      creatorName,
      items: shareItems.map((e) => ({ platformId: e.platformId, gross: e.gross })),
    };
    shareViewData = {
      creatorName,
      period: new Date().getFullYear() + '년 ' + (new Date().getMonth() + 1) + '월',
      platforms: shareItems.map((e) => ({
      ...
      totals: {
        net: shareItems.reduce((s, e) => s + (e.net || 0), 0),
      },
    };
  }

  async function createShareLink() {
    if (!sharePayload || !calcProof) {
      showConnError('다시 계산 후 공유하세요.');
      return;
    }
    const pids = sharePayload.items.map((i) => i.platformId);
    if (!oauthReadyForShare(pids)) {
      showConnError('Instagram·TikTok 로그인 연결 후 공유할 수 있습니다.');
      return;
    }
    ...
    try {
      const data = await apiFetch('/api/share/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Session-Id': sessionId() },
        body: JSON.stringify({ ...sharePayload, calcProof, sessionId: sessionId() }),
      });
      if (!data.ok) {
        if (msg) {
          msg.textContent = data.message || '링크 생성 실패';
          msg.classList.remove('hidden');
          msg.style.color = '#f04452';
        }
        return;
      }
      const url = data.shareUrl || `${location.origin}${data.sharePath || ''}`;
      if (openLink) {
        openLink.href = url;
      ...
      if (msg) {
        ...
        msg.classList.remove('hidden');
        msg.style.color = '#3182f6';
      }
      if (socialSlot && shareViewData && window.SocialShare) {
        socialSlot.classList.remove('hidden');
        SocialShare.renderBar(socialSlot, shareViewData, url);
      }
      try {
      ...
      if (btn) {
        btn.disabled = false;
        btn.textContent = '수익 인증 링크 만들기';
      }
    }
  }

  function applyCalcResult(data) {
    calcProof = data.calcProof || null;
    if (data.oauthStatus) oauthStatusMap = data.oauthStatus;
    renderOAuthBar();
    ...
    const items = (data.entries || []).map((e) => ({
      ...
      platformFee: e.platformFee,
      withholding: e.withholding,
      withholdingLabel: e.calc?.withholdingLabel || e.withholdingLabel,
      net: e.net,
    }));
    const totals = {
      net: s.ytdNet,
    ...
    renderSharePanel(data.entries || [], data.creatorName);
  ...
  async function runCalc() {
    ...
    if (!data.ok) {
      const msg = data.message || '계산할 수 없습니다.';
      showConnError(msg);
    ...
    if (data.usage?.nameRemainingToday != null) {
      ...
      if (banner) {
        banner.innerHTML = `<div class="pt-alert"><strong>이용 안내</strong><p>${note} · <a href="/terms">약관</a></p></div>`;
      }
    }
  }
      ...
      .map(
        (p) =>
          `<button type="button" class="pt-platform-btn ${selectedPlatforms.has(p.id) ? 'active' : ''}" data-pid="${p.id}">
            ${PlatformIcons.iconBadge(p.id, 44)}<span>${PlatformIcons.meta(p.id).label}</span></button>`
      )
      .join('');
    renderAmountInputs();
  }
      ...
      .map(
        (pid) =>
          `<div class="pt-amount-row">
            ${PlatformIcons.iconBadge(pid, 32)}
            <span class="pt-amount-label">${PlatformIcons.meta(pid).label}</span>
            <input type="number" class="pt-input pt-amount-input" data-pid="${pid}" placeholder="이번 달 수익 (원)" min="1" />
          </div>`
      )
      .join('');
    runPreview();
  ...
  document.getElementById('entry-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showConnError('계산 실패. 서버 연결을 확인하세요.');
    }
  });

  document.getElementById('btn-reset')?.addEventListener('click', resetAll);

  document.getElementById('btn-oauth-instagram')?.addEventListener('click', () => connectOAuth('instagram'));
  document.getElementById('btn-oauth-tiktok')?.addEventListener('click', () => connectOAuth('tiktok'));

  document.getElementById('btn-share')?.addEventListener('click', async () => {
    try {
      await createShareLink();
    } catch {
      showConnError('공유 링크 생성 실패');
    }
  });
